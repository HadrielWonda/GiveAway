...
func Test(t *testing.T) {
	stub := shimtest.NewMockStub("cc", new(giveawayChaincode))
	response := stub.MockInit("init", nil)
	t.Logf("Init status code %d", response.Status)
	if response.Status != shim.OK {
		t.FailNow()
	}
	ccargs := SetupArgsArray("userRegistration", "Hadrielwonda", "Pritam", "pw")
	stub.MockInvoke("in", ccargs)

	ccargs = SetupArgsArray("userGateway", "userLogin", "Hadrielwonda", "pw")
